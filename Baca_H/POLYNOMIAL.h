...
{
    stream>>wielomian.stopien;
    if ( wielomian.stopien >= 0)
    {
        wielomian.head = new wsp();
        stream>>wielomian.head->wspolczynnik;

        wsp * prev= wielomian.head;
...
POLYNOMIAL dodaj ( POLYNOMIAL &second,POLYNOMIAL &first)
{
    ...
    for ( unsigned int i = 0;i <= x  ;i++)
    {
        ...
        if ( f != NULL)
        {
            ...
            curr->wspolczynnik += f->wspolczynnik;
            curr->licznik += f->licznik * (wspolny_mianownik / f->mianownik);
            f = f->next;
        }
        if ( s != NULL)
        {
            curr->wspolczynnik += s->wspolczynnik;
            curr->licznik += s->licznik * (wspolny_mianownik /s->mianownik);
            s = s->next;
        }
...
POLYNOMIAL POLYNOMIAL ::operator/ (const POLYNOMIAL& second)
{
    ...
    if ( second.stopien == 0)
    {
        ...
        if ( second.head->wspolczynnik == 0)return POLYNOMIAL();
    }
    unsigned int rozmiar = this->stopien -  second.stopien;
    POLYNOMIAL result(rozmiar);
    POLYNOMIAL rest (*this);
