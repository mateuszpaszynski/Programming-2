...
void run() {
	...
	// 1. Zasadź drzewo w ogrodzie
	garden->plantTree();
	TREE_CLASS* gardenTree = garden->getTreePointer(0);


	BRANCH_CLASS* externalBranch = new BRANCH_CLASS();
	externalBranch->growthBranch();  // Długość 1
	externalBranch->growthBranch();  // Długość 2 → dodaje owoc
	externalBranch->growthBranch();
	externalBranch->growthBranch();

	// Zrob miejsce na galaz w tym drzewie
	gardenTree->growthTree();
	gardenTree->growthTree();
	gardenTree->growthTree();
	gardenTree->growthTree();
	gardenTree->growthTree();
	gardenTree->growthTree();
	print(gardenTree);
	// 3. Klonuj gałąź z zewnętrznego drzewa do drzewa w ogrodzie
	gardenTree->cloneBranch(externalBranch);
	print(gardenTree);

	// 4. Sklonuj całe drzewo
	print(garden);
	garden->cloneTree(0);  // zakładamy, że gardenTree ma numer 0
	print(garden);
	// 5. Usuń oryginalne drzewo
	garden->extractTree(0);

	print(garden);

	// 7. Sprzątanie
	delete externalBranch;
	delete garden;
}
